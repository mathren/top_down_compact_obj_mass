...
if __name__ == "__main__":
    ...
    with open(outfile,'w') as f:
        ...
        f.writelines(r"\label{eq:fit}"+"\n")
        f.writelines(fit.replace("$","")+"\n")
        f.writelines(r"\end{equation}"+"\n")
        f.writelines(r"\end{widetext}"+"\n")
    # now plot
    for i, metallicity in enumerate(sorted(np.unique(Z))):
        ...
        ax.scatter(x, y, color=rainbow[i], s=100, marker='x',
                   label=r"$Z=" + f"{metallicity:.0e}" + "$")
        # then plot fit
